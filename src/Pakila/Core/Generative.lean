...
/-- 画像編集アクションを処理する -/
def handleEditImage (cont : Continuation IO) (nextS : InterpreterState) (file : String) (prompt : String) : IO Unit := do
...
/-- 画像復元アクションを処理する -/
def handleRestoreImage (cont : Continuation IO) (nextS : InterpreterState) (file : String) (_prompt : String) : IO Unit := do
...
/-- 画像生成全般アクションを処理する -/
def handleGenerateImage (cont : Continuation IO) (nextS : InterpreterState) (prompt : String) : IO Unit := do
...
/-- アイコン生成アクションを処理する -/
def handleGenerateIcon (cont : Continuation IO) (nextS : InterpreterState) (prompt : String) (_sizes : List Nat) : IO Unit := do
...
/-- ダイアグラム生成アクションを処理する -/
def handleGenerateDiagram (cont : Continuation IO) (nextS : InterpreterState) (prompt : String) (diagramType : String) : IO Unit := do
