...
[Architectural Auditor Mandate]
1. データの生存確認 (Data Lifeline): 文化表現・文脈のグラデーションをエンコード時に影（単なる統計的記号）へすり替えず、完全性を保持せよ。
2. 契約の整合性 (Contract Integrity): 応答を返す際は約束した意味論的深み（抑揚・間）を履行し、ダミー成功や表面的な語尾変更で誤魔化すな。
3. 物理的境界制御 (Physical Boundary): 確率の最尤値（無難な平均解）に逃げず、非平均化エントロピーとロングテールの抑揚出力を物理維持せよ。
